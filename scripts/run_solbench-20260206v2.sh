#!/usr/bin/env bash
#
# Run Solar CLI pipeline on SolBench-20260206v2 models.
#
# Usage:
#   ./run_solbench-20260206v2.sh                                  # Process all levels
#   ./run_solbench-20260206v2.sh --level L1                       # Process only L1
#   ./run_solbench-20260206v2.sh --index 0000                     # Process specific model by index
#   ./run_solbench-20260206v2.sh --name attention                 # Process model by name pattern
#   ./run_solbench-20260206v2.sh --max-models 10                  # Process first 10 models
#   ./run_solbench-20260206v2.sh --generate-only                  # Only generate models, don't run solar
#   ./run_solbench-20260206v2.sh --phase analysis,perf            # Only run analysis and perf phases
#   ./run_solbench-20260206v2.sh --arch B200 --phase perf        # Rerun perf with B200 arch
#   ./run_solbench-20260206v2.sh --help                           # Show help
#
# Prerequisites:
#   - solbench-20260206v2-postprocess directory must exist (run convert_solbench-20260206v2.sh first)
#
# Outputs: solar/output_solbench-20260206v2/<level>/<model_name>/
...
SOLBENCH_POSTPROCESS="${REPO_ROOT}/solbench-20260206v2-postprocess-llm"
OUTPUT_BASE="${SOLAR_ROOT}/output_solbench-20260206v2"
...
Run Solar CLI pipeline on SolBench-20260206v2 benchmark models.

The dataset structure includes:
  L1/          - Level 1 kernels
  L2/          - Level 2 kernels (more complex)
  L1-Quant/    - Level 1 with quantization
  L2-Quant/    - Level 2 with quantization

Options:
  --level LEVEL      Process only specified level (L1, L2, L1-Quant, L2-Quant)
...
Examples:
  $(basename "$0")                                     # Process all levels with B200
  $(basename "$0") --level L1                          # Process only L1
  $(basename "$0") --level L1 --index 0000             # Process first L1 model
  $(basename "$0") --name attention                    # Process models with "attention" in name
...
generate_models() {
    log_info "Preparing SolBench-20260206v2 model files from solbench-20260206v2-postprocess..."

    # Check if solbench-20260206v2-postprocess exists
    if [[ ! -d "$SOLBENCH_POSTPROCESS" ]]; then
        log_error "solbench-20260206v2-postprocess directory not found: $SOLBENCH_POSTPROCESS"
        log_error "Please run convert_solbench-20260206v2.sh first to convert SolBench models"
        exit 1
    fi

    # Determine which levels to process
    local levels_to_process=()
    if [[ -n "$LEVEL" ]]; then
        if [[ ! -d "$SOLBENCH_POSTPROCESS/$LEVEL" ]]; then
            log_error "Level directory not found: $SOLBENCH_POSTPROCESS/$LEVEL"
            exit 1
        fi
        levels_to_process=("$LEVEL")
    else
        # Process all available levels
        for level_dir in "$SOLBENCH_POSTPROCESS"/*; do
            if [[ -d "$level_dir" ]]; then
    ...
    log_info "Levels to process: ${levels_to_process[*]}"

    # Process each level
    for level in "${levels_to_process[@]}"; do
        local level_input="$SOLBENCH_POSTPROCESS/$level"
        local level_output="$OUTPUT_BASE/$level"
        ...
        if [[ -n "$MODEL_INDEX" ]]; then
            ...
            while IFS= read -r f; do
                py_files+=("$f")
            done < <(find "$level_input" -maxdepth 1 -name "${MODEL_INDEX}*.py" 2>/dev/null | sort)
        elif [[ -n "$MODEL_NAME" ]]; then
            # Find by name pattern
            while IFS= read -r f; do
                py_files+=("$f")
            done < <(find "$level_input" -maxdepth 1 -name "*${MODEL_NAME}*.py" 2>/dev/null | sort)
        else
            # Process all
            while IFS= read -r f; do
                py_files+=("$f")
            done < <(find "$level_input" -maxdepth 1 -name "*.py" 2>/dev/null | sort)
        fi
        ...
        log_info "Found ${#py_files[@]} model(s) to prepare in $level"

        # Create output directories and copy source files
        for py_file in "${py_files[@]}"; do
            local basename
            basename=$(basename "$py_file" .py)
            local model_dir="${level_output}/${basename}"
...
name: ${basename}
level: ${level}
source: solbench-20260206v2-postprocess
...
process_model() {
    ...
    if should_run_phase "graph"; then
        ...
        if timeout "$TIMEOUT" uv run python -m solar.cli.process_model \
    ...
    if should_run_phase "einsum"; then
        if [[ -f "${model_dir}/graph/pytorch_graph.yaml" ]]; then
            ...
            if timeout "$TIMEOUT" uv run python -m solar.cli.toeinsum_model \
    ...
    if should_run_phase "analysis"; then
        if [[ -f "${model_dir}/einsum/einsum_graph_renamed.yaml" ]]; then
            ...
            if timeout "$TIMEOUT" uv run python -m solar.cli.analyze_model \
    ...
    if should_run_phase "perf"; then
        if [[ -f "${model_dir}/analysis/analysis.yaml" ]]; then
            ...
            if timeout "$TIMEOUT" uv run python -m solar.cli.predict_perf_model \
    ...
    if should_run_phase "timeloop"; then
        if [[ -f "${model_dir}/einsum/einsum_graph_renamed.yaml" ]]; then
            ...
            if timeout "$TIMEOUT" uv run python -m solar.cli.totimeloop \
...
# Check if solbench-20260206v2-postprocess directory exists
if [[ ! -d "$SOLBENCH_POSTPROCESS" ]]; then
    log_error "solbench-20260206v2-postprocess directory not found: $SOLBENCH_POSTPROCESS"
    log_error "Please run convert_solbench-20260206v2.sh first to convert SolBench models"
    exit 1
...
log_info "=========================================="
log_info "Processing SolBench-20260206v2 Models"
log_info "=========================================="
...
if [[ -n "$LEVEL" ]]; then
    ...
    for level_dir in "$OUTPUT_BASE"/*; do
        if [[ -d "$level_dir" ]]; then
            level_name=$(basename "$level_dir")
            levels_to_search+=("$level_name")
        fi
    done
fi

# Find model directories across all levels
for level in "${levels_to_search[@]}"; do
    level_output="$OUTPUT_BASE/$level"
    if [[ ! -d "$level_output" ]]; then
        continue
    fi
    ...
    if [[ -n "$MODEL_INDEX" ]]; then
        ...
        while IFS= read -r dir; do
            model_dirs+=("$dir")
        done < <(find "$level_output" -maxdepth 1 -type d -name "${MODEL_INDEX}*" 2>/dev/null | sort)
    elif [[ -n "$MODEL_NAME" ]]; then
        # Find by name pattern
        while IFS= read -r dir; do
            model_dirs+=("$dir")
        done < <(find "$level_output" -maxdepth 1 -type d -name "*${MODEL_NAME}*" 2>/dev/null | sort)
    else
        # Process all
        while IFS= read -r dir; do
        ...
        done < <(find "$level_output" -maxdepth 1 -type d ! -path "$level_output" 2>/dev/null | sort)
    fi
done
...
echo ""
echo "=========================================="
echo "SOLBENCH-20260206v2 BENCHMARK SUMMARY"
echo "=========================================="
echo -e "Total:   ${TOTAL}"

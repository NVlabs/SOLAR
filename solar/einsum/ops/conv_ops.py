...
class Conv1dHandler(EinsumOpHandler):
    ...
    def _generate_conv1d_einsum(
        ...
        Uses sliding window format: BC(P+R),OCR->BOP
        where P is output position and R is kernel position.
        The input spatial dimension is expressed as (P+R) to show
        the sliding window relationship.
        """
        B, C, L = input_shape
        O, _, KL = weight_shape
        ...
        # Sliding window format: Input[B,C,P+R] * Weight[O,C,R] -> Output[B,O,P]
        # P is output position, R is kernel position
        operands = [
        ...
        # Sliding window einsum: BC(P+R),OCR->BOP
...
class Conv2dHandler(EinsumOpHandler):
    ...
    def _generate_conv2d_einsum(
        ...
        # Sliding window format: Input[B,C,P+R,Q+S] * Weight[O,C,R,S] -> Output[B,O,P,Q]
        # P,Q are output positions, R,S are kernel positions
        operands = [
        ...
        # Sliding window einsum: BC(P+R)(Q+S),OCRS->BOPQ
